...
forecast_date <- as.Date("2025-01-01") 
...
d<-1
weather_future = data.frame()
while(d<=365){
  weather_future_s3 <- neon4cast::noaa_stage2(start_date = as.character(forecasted_dates[d]))
  print(1)
  temp_weather <- weather_future_s3 |> 
    dplyr::filter(datetime >= forecasted_dates[d],
                  site_id %in% focal_sites,
                  variable %in% met_variables) |> 
    collect()
  print(2)
  filtered_weather <- temp_weather |> 
    filter(datetime <= forecasted_dates[d]+4)
  weather_future <- rbind(weather_future, filtered_weather)
  print(3)
  d <- d+5
}
...
# ------- Grab all observations for comparison in reforecast----------
weather_2025 <- weather_past_daily |> 
  mutate(Year = format(datetime, "%Y")) |> 
  filter(Year == 2025,
         site_id == "BARC")

ggplot(weather_2025, aes(x = datetime, y = air_temperature)) + geom_point()
...
for(i in 1:length(focal_sites)) {  
  ...
  combined_weather <- bind_rows(filtered_historical_weather, noaa_future_site) |> 
    group_by(parameter) |> 
    mutate(alag1 = lag(air_temperature, n=1),
           lag2 = lag(air_temperature, n=2),
           lag3 = lag(air_temperature, n=3),
           three_mean = (alag1 + lag2 + lag3)/3,
  ...
  for (t in 1:length(forecasted_dates)) {
    ...
    if(as.Date(targets_lm$datetime[nrow(targets_lm)]) == forecast_date){
      initD <- data.frame(prediction = rnorm(n_members, mean = targets_lm$lag1[nrow(targets_lm)], sd = params_se[4]), ensemble = seq(1:n_members))
    }else{
      initD <- data.frame(prediction = rnorm(n_members, mean = 0, sd = 1), ensemble = seq(1:n_members))
    }
...
targets_lm_daily <- targets_lm |> 
  group_by(datetime) |> 
  summarise(prediction = mean(temperature))

ggplot() +
  geom_line(data = forecast_df_EFI,aes(x=datetime, y=prediction, group = parameter)) +
  geom_point(data = targets_lm_daily, aes(x = datetime, y=prediction)) +
  facet_wrap(~site_id) +
